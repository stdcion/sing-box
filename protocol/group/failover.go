...
func NewFailover(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.FailoverOutboundOptions) (adapter.Outbound, error) {
	outbound := &Failover{
		...
		lastResort:                   options.LastResort != nil && *options.LastResort,
...
func (s *Failover) Start() error {
	...
	group, err := NewFailoverGroup(s.ctx, s.outbound, s.logger, outbounds, s.link, s.interval, s.idleTimeout, s.recoveryThreshold, s.failureThreshold, s.lazyHealthCheck, s.lastResort, s.interruptExternalConnections)
	if err != nil {
		return err
	}
	s.group = group
...
func (g *FailoverGroup) urlTest(ctx context.Context, force bool) (map[string]uint16, error) {
	...
	for i, detour := range g.outbounds {
		...
		if i == len(g.outbounds)-1 && g.lastResort && !force {
			continue
		}
		tag := detour.Tag()
	...
	if g.lastResort {
		...
		g.failureCounts[lastIdx].Store(0)
		g.recoveryCounts[lastIdx].Store(0)
	}
